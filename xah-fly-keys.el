     ...
     ((string-match-p "\\`https?://" $path)
      (if (fboundp 'xahsite-url-to-filepath)
          (let (($x (xahsite-url-to-filepath $path)))
            (funcall (if (string-match "^http" $x) 'browse-url 'find-file)
                     $x))
        (browse-url $path)))
     ;; not starting “http://”
     ((string-match "#" $path)
      (let (($fpath (substring $path 0 (match-beginning 0)))
        ...
        (cond
         ...
         ((y-or-n-p (format "file no exist: 「%s」. Create?" $fpath))
          (find-file $fpath)))))
     ...
     ((file-exists-p $path)
      ...
      (let (($ext (file-name-extension $path))
        ...
        (if (and (string-equal $ext "js")
          ...
          (find-file $path))))
     ((file-exists-p (concat $path ".el"))
      (find-file (concat $path ".el")))
     ((y-or-n-p (format "file no exist: 「%s」. Create?" $path))
      (find-file $path)))))
